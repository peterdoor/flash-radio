...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Si no hay música, devolver voz directo
    if (musica === 'none') {
      res.setHeader('Content-Type', 'audio/mpeg');
      return res.status(200).send(vozBuffer);
    }

    // 3. Mezclar con FFmpeg
    ...
    try { execSync('which ffmpeg || ffmpeg -version', { stdio: 'ignore' }); }
    catch(e) {
      // FFmpeg no disponible — devolver solo la voz
      console.log('FFmpeg no disponible, devolviendo voz sin música');
      res.setHeader('Content-Type', 'audio/mpeg');
      return res.status(200).send(vozBuffer);
    }

    // Verificar que intro/outro existen
    if (!fs.existsSync(introPath) || !fs.existsSync(outroPath)) {
      console.log('intro/outro no encontrados, devolviendo voz sin música');
      res.setHeader('Content-Type', 'audio/mpeg');
      return res.status(200).send(vozBuffer);
    }
    ...
    try {
      execSync(
        `ffmpeg -y -i "${introPath}" -i "${vozPath}" -i "${outroPath}" ` +
      ...
      // FFmpeg falló — devolver voz sin música como fallback
      console.error('FFmpeg error:', ffmpegErr.message?.slice(0,200));
      res.setHeader('Content-Type', 'audio/mpeg');
      return res.status(200).send(vozBuffer);
    } finally {
