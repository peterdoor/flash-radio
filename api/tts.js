    ...
    if (proveedor === 'google') {
      ...
      const r = await fetch(`https://texttospeech.googleapis.com/v1/text:synthesize?key=${key}`, {
        ...
        body: JSON.stringify({
          input: { text: texto },
          voice: {
            languageCode: voz ? voz.substring(0,5) : 'es-US',
            name: voz || 'es-US-Neural2-B',
            ssmlGender: 'MALE'
          },
          audioConfig: {
            audioEncoding: 'MP3',
      ...
      const r = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`, {
        ...
        body: JSON.stringify({
          ...
          voice_settings: { stability: 0.30, similarity_boost: 0.75, style: 0.2, use_speaker_boost: true, speed: 1.20 }
        })
      });
